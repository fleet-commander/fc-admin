...
class TestFCAD(unittest.TestCase):
    ...
    def _get_domain_profile(self, cn):
        ...
        return fcad.ldap.DOMAIN_DATA['profiles'][key]

    def _get_domain_profile_data(self, cn, template=TEST_PROFILE_DOM_DATA):
        dom_profile = template.copy()
        dom_profile['cn'] = (cn, )
        dom_profile['gPCFileSysPath'] = (dom_profile['gPCFileSysPath'][0] % cn, )
        return dom_profile        
...
# if __name__ == 'PREVIOUS_MANUAL_TESTS':
#     import pprint
...
#     connector = fcad.ADConnector('FC.AD')
