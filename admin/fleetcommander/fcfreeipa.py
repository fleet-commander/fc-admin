...
class FreeIPAConnector:
    ...
    def _create_profile_rules(self, profile):
        ...
        users = list(map(str, profile["users"]))
        groups = list(map(str, profile["groups"]))
        logger.debug(
        ...
        if "hostcategory" not in parms:
            hosts = list(map(str, profile["hosts"]))
            hostgroups = list(map(str, profile["hostgroups"]))
            logger.debug(
    ...
    def _update_profile_rules(self, profile, oldname=None):
        ...
        api.Command.deskprofilerule_add_user(
            name,
            user=list(map(str, udif)),
            group=list(map(str, gdif)),
        )
        ...
        if parms["hostcategory"] == "all":
            api.Command.deskprofilerule_remove_host(
                name,
                host=list(map(str, applies["hosts"])),
                hostgroup=list(map(str, applies["hostgroups"])),
            )
        else:
            ...
            api.Command.deskprofilerule_add_host(
                name,
                host=list(map(str, hdif)),
                hostgroup=list(map(str, hgdif)),
            )
